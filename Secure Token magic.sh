...
# The Other Guy - 01/23/45 - V1.3
# Added expect statement specifically for those that want to enable FileVault for users that fall under a specific category of problem.
# (Computers that defer enrollment of FileVault to the admin account that has the secure Token)
...
# Notification banner stating that Newport Academy needs you to enable FileVault
osascript -e 'display notification "Incoming Message from IT Support." with title "Newport Academy" sound name "Glass"

delay 5

tell application "System Events" to display dialog "Newport Academy IT needs to enable FileVault on your computer.

If you have any questions or concerns, please reach out to the Help Desk by calling our support number:  866-611-2828" with title "Newport Academy FileVault Encryption" with icon POSIX file "/Library/Application Support/NewportAcademy/NAicon512.png" buttons {"Ok"} default button 1 with text'

# Sleep for a bit to make things look more legit and less like a virus
...
# Check if the admin provided exists on the system
        ...
        echo "Admin user status: You were right! the account did exist"
fi
...
# Check Secure Tokens Status - Do we have any Token Holder?
		...
		echo "Token Status $tokenStatus"
				...
				userPass="$(osascript -e 'Tell application "System Events" to display dialog "Please enter your current password" default answer "" with title "FileVault Configuration" with text buttons {"Ok"} default button 1 with hidden answer' -e 'text returned of result')"
								...
								echo "asking again"
								userPassAgain="$(osascript -e 'Tell application "System Events" to display dialog "Password Incorrect, please try again." default answer "" with title "Login Password" with text buttons {"Ok"} default button 1 with hidden answer' -e 'text returned of result')"
                ...
                # Make the computer talk to itself for a moment so it can figure things out
                
                if [ -f /Library/Keychains/FileVaultMaster.keychain ]; then
                	echo "User Has Master Keychain! Running enablement in User Token True block."
                    	expect -c '
                          spawn fdesetup enable -keychain -user '$userName'
                	...
                	echo "Running enablement within User Token True block. Also User does not have Master Keychain."
                      expect -c '
                          spawn fdesetup enable -user '$userName'
                          expect "Enter the password for the user '$userName':"
                          send "'$userPass'\r"
                          expect'
				...
				userPass="$(osascript -e 'Tell application "System Events" to display dialog "Please enter your current password" default answer "" with title "FileVault Configuration" with text buttons {"Ok"} default button 1 with hidden answer' -e 'text returned of result')"
								...
								while [[ "$passDSCLCheck" != "0" ]]; do
								echo "asking again"
								userPassAgain="$(osascript -e 'Tell application "System Events" to display dialog "Wrong Password!" default answer "" with title "Login Password" with text buttons {"Ok"} default button 1 with hidden answer' -e 'text returned of result')"
								userPass=$userPassAgain
								passDSCLCheck=`dscl /Local/Default authonly $userName $userPassAgain; echo $?`
								done
...
# if end user has the secure token and is also a FileVault Delinquent, call the FileVault policy.
				if [ -f /Library/Keychains/FileVaultMaster.keychain ]; then
                	echo "User Has Master Keychain! Running enablement in the End Block."
                    	expect -c '
                          spawn fdesetup enable -keychain -user '$userName'
                          expect "Enter the password for the user '$userName':"
                          send "'$userPass'\r"
                	...
                	echo "Running enablement within the End Block. Also User does not have Master Keychain."
                      expect -c '
                          spawn fdesetup enable -user '$userName'
                          expect "Enter the password for the user '$userName':"
                          send "'$userPass'\r"
